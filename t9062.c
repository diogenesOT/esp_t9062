...
int8_t t9062_write_register(t9062_sensor_t *sensor, uint8_t register_id, uint16_t register_data) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "[t9062_write_register()] error %i reading register. register: %s [0x%02X]", ret,
...
int8_t t9062_change_address(t9062_sensor_t *sensor, uint8_t new_address) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "[t9062_change_address()] error %i read register command. register: %s", ret,
                 t9062_register_name[T9062_REG_CUST_CONFIG]);
        return 3;
    }
    if ((register_data & 0x003F) != sensor->address) {
        ESP_LOGE(TAG, "[t9062_change_address()] error sensor address and "
    ...
    if ((register_data & 0x003F) != new_address) {
        ESP_LOGE(TAG, "[t9062_change_address()] error new address and "
                      "register data not matching.");
        return 6;
    }
