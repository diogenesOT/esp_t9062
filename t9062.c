...
int8_t t9062_write_register(t9062_sensor_t *sensor, uint8_t register_id, uint16_t register_data) {
    ...
    // write register
    ret = i2c_master_write_to_device(sensor->i2c_port, sensor->address, cmd, sizeof(cmd), pdMS_TO_TICKS(T9062_I2C_TIMEOUT_MS));
    if (ret != ESP_OK) {
        ESP_LOGE(T9062_LOG_TAG, "error %i sending read register command. register: %s [0x%02X]", ret, t9062_register_name[register_address],
                 register_address);
        return 2;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(T9062_LOG_TAG, "error %i reading register. register: %s [0x%02X]", ret, t9062_register_name[register_address],
                 register_address);
        return 3;
    }
...
int8_t t9062_change_address(t9062_sensor_t *sensor, uint8_t new_address) {
    ...
    ESP_LOGI(T9062_LOG_TAG, "[t9062_change_address()] old register data:[0x%02X] [0x%02X] new register data: [0x%02X] [0x%02X]",
             register_data >> 8, (uint8_t)register_data, cmd[1], cmd[2]);
    ESP_LOGI(T9062_LOG_TAG, "[t9062_change_address()] write register command [0x%02X] [0x%02X] [0x%02X]", cmd[0], cmd[1], cmd[2]);
...
int8_t enter_normal_mode(t9062_sensor_t *sensor) {
    ...
    if ((ret == ESP_OK) & (get_status(ans[0]) < 2)) {
        ...
        ESP_LOGE(T9062_LOG_TAG, "[enter_normal_mode()] error i2c_master_read_from_device returned %i. Data:  0x%02X", ret, ans[0]);
        return -1;
    }
    // enter normal mode
    cmd[0] = 0x80;
    ...
    if ((ret == ESP_OK) & (get_status(ans[0]) < 2)) {
        ...
        ESP_LOGW(T9062_LOG_TAG, "[enter_normal_mode()] error i2c_master_read_from_device returned %i. Data:  0x%02X", ret, ans[0]);
        return 1;
    }
...
int8_t enter_command_mode(t9062_sensor_t *sensor) {
    ...
    */
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(
        i2c_master_write_to_device(sensor->i2c_port, sensor->address, cmd, sizeof(cmd), pdMS_TO_TICKS(T9062_I2C_TIMEOUT_MS)));
    esp_rom_delay_us(105);
    ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_master_read_from_device(sensor->i2c_port, sensor->address, &ans[0], COMMAND_MODE_RETURN_SIZE,
                                                              pdMS_TO_TICKS(T9062_I2C_TIMEOUT_MS)));
